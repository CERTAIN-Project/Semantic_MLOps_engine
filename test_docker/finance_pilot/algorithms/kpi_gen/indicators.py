...
import numpy as np
from finance_pilot.utils.constants import DEFAULT_RATING_COL
...
def roi(df, periods=(1, 21, 63, 126, 189)):
    ...
    """
    for t in periods:
        df[f"past_profitability_{t}d"] = (
            df[DEFAULT_RATING_COL] - df[DEFAULT_RATING_COL].shift(t)
        ) / df[DEFAULT_RATING_COL].shift(t)
    return df
...
def volatility(df, periods=(21, 63, 126, 189)):
    ...
    if "past_profitability_1d" not in df:
        df["past_profitability_1d"] = (
            df[DEFAULT_RATING_COL] - df[DEFAULT_RATING_COL].shift(1)
        ) / df[DEFAULT_RATING_COL].shift(1)
        drop_col = True

    for t in periods:
        if t == 0:
            df[f"volatility"] = df["past_profitability_1d"].expanding(
                min_periods=2
            ).std() * np.sqrt(252)
        else:
            df[f"volatility_{t}d"] = df["past_profitability_1d"].rolling(
                window=t
            ).std() * np.sqrt(252)
    if drop_col:
...
def sharpe(df, periods=(21, 63, 126, 189)):
    for t in periods:
        ...
        df[f"sharpe_{t}d"] = df[f"sharpe_{t}d"].apply(
            lambda x: 0.0 if math.isnan(x) else x
        )
        df[f"sharpe_{t}d"] = df[f"sharpe_{t}d"].apply(
            lambda x: 0.0 if math.isinf(x) else x
        )
    return df
...
def relative_strength_index(df, n=14):
    ...
    df[rsi_name] = 100 - 100 / (
        1
        + df["up"].ewm(span=n, adjust=False).mean()
        / df["down"].ewm(span=n, adjust=False).mean()
    )
...
def detrended_close_oscillator(df, n=22):
    ...
    mid_index = int(n / 2 + 1)
    df[dco_name] = (
        df[DEFAULT_RATING_COL].shift(mid_index)
        - df[DEFAULT_RATING_COL].rolling(window=n).mean()
    )
    return df
