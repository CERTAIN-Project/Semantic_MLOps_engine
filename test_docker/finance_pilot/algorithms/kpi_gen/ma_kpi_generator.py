...
from finance_pilot.algorithms.kpi_gen.indicators import *
from finance_pilot.algorithms.kpi_gen.kpi_generator import KPIGenerator
...
class MAKPIGenerator(KPIGenerator):
    ...
    def compute(self):
        ...
        for asset in assets:
            ...
            asset_time_series_df = avg_price(
                asset_time_series_df,
                periods=(
                    [21, 63, 126] if self.kpi_type == "short" else [21, 63, 126, 189]
                ),
            )
            asset_time_series_df = roi(
                asset_time_series_df,
                periods=(
                    [1, 21, 63, 126]
                    if self.kpi_type == "short"
                    else [1, 21, 63, 126, 189]
                ),
            )
            asset_time_series_df = volatility(
                asset_time_series_df,
                periods=(
                    [21, 63, 126] if self.kpi_type == "short" else [21, 63, 126, 189]
                ),
            )
            asset_time_series_df = moving_average_convergence_divergence(
                asset_time_series_df
            )
            asset_time_series_df = momentum(
                asset_time_series_df,
                periods=(
                    [21, 63, 126] if self.kpi_type == "short" else [21, 63, 126, 189]
                ),
            )
            asset_time_series_df = rate_of_change(
                asset_time_series_df,
                periods=(
                    [21, 63, 126] if self.kpi_type == "short" else [21, 63, 126, 189]
                ),
            ...
            asset_time_series_df = sharpe(
                asset_time_series_df,
                periods=(
                    [21, 63, 126] if self.kpi_type == "short" else [21, 63, 126, 189]
                ),
            )
            asset_time_series_df = min_max(
                asset_time_series_df,
                periods=(
                    [21, 63, 126] if self.kpi_type == "short" else [21, 63, 126, 189]
                ),
