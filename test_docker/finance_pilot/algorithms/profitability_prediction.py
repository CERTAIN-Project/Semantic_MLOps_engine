...
from finance_pilot.algorithms.algorithm import Algorithm
...
class ProfitabilityPrediction(Algorithm):
    ...
    def train(self, train_date):
        ...
        for asset in kpi_indicators[DEFAULT_ITEM_COL].unique():
            ...
            asset_df["final_price"] = asset_df[DEFAULT_RATING_COL].shift(
                -self.months * 21
            )
            asset_df["target"] = (
                asset_df["final_price"] - asset_df[DEFAULT_RATING_COL]
        ...
        kpi_indicators = kpi_indicators[
            kpi_indicators[DEFAULT_TIMESTAMP_COL] < (train_date - delta)
        ]
    ...
    def recommend(self, rec_time, repeated, only_test_customers):
        ...
        kpi_indicators = kpi_indicators[
            kpi_indicators[DEFAULT_TIMESTAMP_COL] == rec_time
        ]
        kpi_indicators = kpi_indicators[
            kpi_indicators[DEFAULT_ITEM_COL].isin(self.data.assets)
        ]

        # Then, we obtain the recommendation scores:

        if self.is_fitted:
            kpi_indicators["score"] = self.model.predict(
                kpi_indicators.drop(columns=[DEFAULT_ITEM_COL, DEFAULT_TIMESTAMP_COL])
            )
        else:
            kpi_indicators["score"] = kpi_indicators[DEFAULT_ITEM_COL].apply(
                lambda x: random.random()
            )

        # And, finally, we sort the assets by score:
        kpi_indicators = kpi_indicators[[DEFAULT_ITEM_COL, "score"]].sort_values(
        ...
        for customer in customers:
            ...
            if not repeated:
                items_per_user = set(
                    self.data.train[self.data.train[DEFAULT_USER_COL] == customer][
                        DEFAULT_ITEM_COL
                    ]
                    .unique()
                    .flatten()
                ...
                user_recommendation = user_recommendation[
                    ~user_recommendation[DEFAULT_ITEM_COL].isin(items_per_user)
                ]
